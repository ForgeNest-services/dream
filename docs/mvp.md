...
**Note:** the lifecycle below applies independently to *each module* a tenant has (per `module_subscriptions`), not once per tenant. A tenant can simultaneously have Core PMS in `active` and Restaurant POS in `trial` — these statuses never merge or influence each other. The state machine itself is identical per module; what differs is that a tenant now has one instance of it per module rather than one overall.
...
## 3. Subscription Plans & Pricing — Modular, Per-Module Billing

Pricing is **module-based**, not one flat all-inclusive plan. Core PMS is the mandatory foundation every tenant needs to operate at all; every additional module (Restaurant POS, and whatever comes after it) is sold, trialed, and billed **independently**. This matters the moment a second module exists — a hotel using only Core PMS should never be paying for Restaurant POS capacity it doesn't use, and a hotel that adds Restaurant POS later shouldn't have that reset or complicate their existing Core PMS billing.

### Core PMS (mandatory base module)

| Plan | Price | Billing cycle | Notes |
|---|---|---|---|
| Trial | Free | 30 days, one-time per tenant | Full Core PMS features. One trial per business — guard against re-signup abuse via PAN/VAT or phone number matching |
| Monthly | NPR 2,999/month | Recurring monthly | |
| Yearly | NPR 11,999/year | Recurring yearly | ~33% cheaper than monthly × 12 — pushes yearly commitment, matches local B2B norms better than monthly auto-billing |
| Branch add-on | Flat fee per additional branch/month (e.g. NPR 1,500) | Same cycle as base plan | Applies from the 2nd branch onward; covers Core PMS only for that branch — each additional module below is billed separately per branch too |

### Add-on modules (e.g. Restaurant POS)

| Plan | Price | Billing cycle | Notes |
|---|---|---|---|
| Trial | Free | 30 days per module, independent of Core PMS trial status | A tenant already paying for Core PMS for months still gets a fresh 30-day trial the first time they activate a new module |
| Monthly | e.g. NPR 999–1,500/month, per property | Recurring monthly | Cheaper than Core PMS, reflecting narrower scope — still flat-fee, not usage-metered |
| Yearly | Discounted equivalent of monthly × 12 | Recurring yearly | Same ~33% yearly discount pattern as Core PMS, for consistency |

**Still no usage-based or per-transaction pricing within a module** — once a hotel subscribes to a module, it's flat regardless of how much they use it, consistent with the original "simple, trustworthy" pricing philosophy. What's changed is that *which modules* a tenant pays for is now their choice, not bundled.

**Status independence is the core rule:** Core PMS can be `active` while Restaurant POS is `trial`, `expired`, or never activated at all — and vice versa. No module's billing status may ever be inferred from another's.

---

## 4. Feature Gating Rules

Gating is evaluated **per module** — a lapsed Restaurant POS subscription must never disable Core PMS features, and vice versa. Each module defines its own "stays available vs. disabled" split for its `trial_expired`/`suspended` states.
...
### Restaurant POS (and future modules)
Each new module needs this same split defined explicitly before launch — e.g., for Restaurant POS, order-taking might stay available (operational continuity) while kitchen analytics/reports get gated. Don't assume the Core PMS split applies by default; decide it deliberately per module.
