...
const ROLES = [
  { code: "manager", label: "Manager", scoped: true, description: "Runs day-to-day operations at one branch: menu, orders, tables, and reports for that branch only." },
  { code: "waiter", label: "Waiter", scoped: true, description: "Takes and serves orders at one branch. No access to reports or settings." },
  { code: "chef", label: "Chef", scoped: true, description: "Views and updates order status in the kitchen at one branch. No access to reports or settings." },
] as const;
...
export function UsersSection() {
  ...
  // Owner-only feature — waiters/managers/chefs never see this section at all.
    ...
    <div className="pos-card p-5">
      ...
      <div className="mt-4 space-y-4">
        {branches.length === 0 ? (
          ...
          ROLES.map((r) => {
            const slots: Slot[] = branches.map((b) => ({
              role: r.code,
              branchId: b.id,
              branchName: b.name,
            ...
            return (
              <div key={r.code} className="overflow-hidden rounded-xl border border-border">
                <div className="flex items-start justify-between gap-3 border-b border-border bg-secondary/40 p-4">
                  <div>
                    <div className="flex items-center gap-2">
            ...
            <AlertDialogAction
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  toast.error(e instanceof ApiError ? e.message : "Failed to remove login");
                }
              }}
            >
              Remove
