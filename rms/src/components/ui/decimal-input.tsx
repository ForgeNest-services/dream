...
export function DecimalTextInput({
  ...
  useEffect(() => {
    // Resync when the value changes from outside (e.g. the linked Excl./Incl.
    // VAT field, or loading a different item) — but never while this field
    // itself is focused. A linked Excl./Incl. VAT pair round-trips a typed
    // value through a 2-decimal-place STORED intermediate (the other
    // field), which is lossy by up to a full cent (e.g. typing 5 into
    // Incl. VAT at 13% stores Excl. VAT as 4.42, which converts back to
    // 4.99, not 5) — not just float noise, so a plain 2dp-comparison guard
