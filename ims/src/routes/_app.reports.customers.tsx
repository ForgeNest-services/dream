...
interface CustomerStatementSearch {
  q: string;
  from: string;
  to: string;
  page: number;
...
export const Route = createFileRoute("/_app/reports/customers")({
  head: () => ({
    meta: [
      { title: "Customer Statement — SROTA IMS" },
      {
        name: "description",
        content: "Every customer's outstanding balance and activity in the selected period.",
      },
    ],
  }),
  validateSearch: (search: Record<string, unknown>): CustomerStatementSearch => {
