...
function SuppliersPage() {
  ...
  const rows = useMemo(() => {
    ...
    return app.parties
      ...
      .map((p) => {
        const purchases = app.movements.filter((m) => m.supplierId === p.id);
        const purchased = purchases.reduce((s, m) => s + (m.unitCost ?? 0) * m.qty, 0);
        return {
          party: p,
          deliveries: purchases.length,
          purchased,
  ...
  return (
    <div>
      <PageHeader
        title="Suppliers"
        subtitle="Vendor accounts, goods received and outstanding payables."
        actions={
          <>
            <Button
              variant="outline"
      ...
      <CustomerDialog open={addOpen} onOpenChange={setAddOpen} kind="supplier" />
