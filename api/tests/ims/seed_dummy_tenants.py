"""One-time bootstrap: creates two real dummy IMS tenants (one PAN-only, one
VAT-registered) with a branch and all 3 role credentials each, for the
ordered black-box HTTP test suite in this directory.

NOT a pytest test itself -- run once, manually, inside the api container:
    docker exec srota-api python /app/tests/ims/seed_dummy_tenants.py
...
Mirrors api/tests/rms/seed_dummy_tenants.py exactly, adapted for IMS's own
credential table/service (features.ims.service.IMSCredentialService) and
role set (owner/manager/storekeeper -- no waiter/chef equivalent).
"""
import sys
import json
...
from features.ims.service import IMSCredentialService
from features.ims.roles import IMSRole

PAN_TENANT_NAME = "IMS Test PAN Pvt Ltd"
VAT_TENANT_NAME = "IMS Test VAT Pvt Ltd"
...
def _get_or_create_credential(db, tenant_id: str, created_by: str, branch_id: str | None, role: str, username: str):
    ...
    existing = IMSCredentialRepository.get_by_username(db, username)
    if existing:
...
def seed_business(db, business_key: str, tenant_name: str, pan: str | None, is_vat_registered: bool) -> dict:
    ...
    owner_user = _get_or_create_owner_user(db, tenant.id, f"ims-{business_key}-owner@test.dream")
    branch = _get_or_create_branch(db, tenant.id, "Main Branch", f"IMS{business_key.upper()}01")

    creds = {}
    for role in IMSRole.values():
        username = f"ims-{business_key}-{role}"
        branch_id = branch.id if role != IMSRole.OWNER.value else None
...
def main_seed():
    db = SessionLocal()
    try:
        pan_business = seed_business(db, "pan", PAN_TENANT_NAME, pan="600000201", is_vat_registered=False)
        vat_business = seed_business(db, "vat", VAT_TENANT_NAME, pan="600000202", is_vat_registered=True)

        output = {"pan": pan_business, "vat": vat_business}
        out_path = "/app/tests/ims/dummy_tenants.json"
