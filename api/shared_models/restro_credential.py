...
class RestroCredential(Base):
    """Staff login for Zestro (restro). owner is tenant-wide (branch_id=NULL,
    sees every branch); manager/waiter/chef are branch-scoped — multiple
    people can share a role at the same branch (e.g. two waiters at KTM).
    `username` is globally unique and used only for login. `name` is the
    display name shown on printed bills and the activity log."""

    __tablename__ = "restro_credentials"
    ...
    def __repr__(self):
        return f"<RestroCredential(tenant_id={self.tenant_id}, branch_id={self.branch_id}, role={self.role})>"
