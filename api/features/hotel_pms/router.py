    ...
    if not result["success"]:
        ...
        if code == "ROLE_ALREADY_HAS_CREDENTIAL":
            return error_response(
                ...
                f"A credential for role '{data.role}' already exists.",
                409,
            )
...
@router.patch("/credentials/{role}", dependencies=owner_dep)
def update_credential(
    ...
    if not result["success"]:
        ...
        if code == "CREDENTIAL_NOT_FOUND":
            return error_response("CREDENTIAL_NOT_FOUND", "No credential for that role.", 404)
        if code == "USERNAME_TAKEN":
...
@router.delete("/credentials/{role}", dependencies=owner_dep)
def delete_credential(
    ...
    user = current_user["user"]
    result = HotelPMSCredentialService.delete(db, tenant_id=user.tenant_id, role=role)
